...
const CustMenu = () => {
  ...
  const onclickAdd = (el) => {
    ...
    const itemIndex = wishlist.findIndex((item) => item._id === el._id);
    console.log("element you clicked on:",el)
    if (itemIndex === -1) {
  ...
  return (
    <>
      ...
      {!showOrder ? <div className="menu">
        ...
        <div className='orders grid grid-cols-1 gap-4 p-4'>
          {orders.map((order) => (
            <div key={order._id} className='order bg-white shadow-md rounded-lg p-4'>
              <h4 className='text-lg font-semibold mb-2'>Restaurant: {order.restaurant.name}</h4>
              ...
              <p className='text-gray-700 mb-1'><strong>Delivery Address:</strong> {order.deliveryAddress}</p>
              <p className='text-gray-700'><strong>Order Date:</strong> {new Date(order.createdAt).toLocaleDateString()}</p>
            </div>
          ))}
        </div>
      }
